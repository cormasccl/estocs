...
getWPEditor : function(content){
	...
	jQuery.post(ajaxurl,{ type:'rad_wp_editor' , action:'rad_query_listener'  },function(data){
		jQuery('.r-w-editor-area').html(data);
		EDITOR_FLAG = true;
...
if(  typeof operation == "undefined"  )
    ...
    if(  typeof el.data('property') != "undefined" && el.data('property')!="" )
    {
    	if( jQuery.inArray(el.data('property'), px_keys) > -1 ) value +="px";
    	 console.log(el.data('property')+" => "+value);
...
if(Modernizr.localstorage && Phantom.checkRegistry('RAD_Live_Version') )
{
	...
	{
		Phantom.reset = true;
		setTimeout(function(){ Phantom.reset = false; 
		Phantom.addLocalRegistry('RAD_Live_Version',RAD_Live_Version); },6000);
	}
...
var rad_live_builder = {
	...
	init : function() {
		    ...
		    frame.droppable({
			      ...
			      drop: function( event, ui ) {
			       ...
			       if( ui.draggable.data('group') === "widgets"  ||  ui.draggable.data('group') === "advance"   ||  ui.draggable.data('group') === "media"  || ui.draggable.data('group') === "plugins" || ui.draggable.data('group') === "core" )
			        {
	...
	containerHandle : function(obj){
       ...
       obj.droppable({
                          accept: " .cl-widgets,.cl-advance,.cl-media ,.rad_page_widget,.cl-plugins",
                          greedy: true,
   ...
   sectionHandle : function(obj) {
        obj.droppable({
                      accept: ".cl-structure , .cl-widgets,.cl-advance,.cl-media , .rad_page_container,.cl-plugins", 
                      hoverClass: "rad-section-dropping",
                      greedy:true,
                      drop: function( event, ui ) {
                          if( ui.draggable.data('group') === "widgets" ||  ui.draggable.data('group') === "advance"   ||  ui.draggable.data('group') === "media"  || ui.draggable.data('group') === "core" || ui.draggable.data('group') === "plugins")
                          {

 ...
 */
	public function index($page = 1, $partner_id = null) {
        ...
        $query_count  = "SELECT count( Stocks.id) AS count  FROM `stocks` Stocks LEFT JOIN articles Articles ON Stocks.`article_id` = Articles.id LEFT JOIN products Products ON Articles.product_id = Products.id WHERE Stocks.partner_id =".$partner_id;
    ...
    public function home($page = 1) {
        ...
        FROM `stocks` Stocks lEFT OUTER JOIN `articles` Articles ON Articles.id = Stocks.article_id
        LEFT JOIN products Products ON Articles.product_id = Products.id WHERE 1 = 1 ";
    ...
    public function ajaxManageStocksTotalSearch(){
        ...
        $query = array();
        $query['count']  = "SELECT count( Stocks.id) AS count  FROM `stocks` Stocks LEFT JOIN articles Articles ON Stocks.`article_id` = Articles.id LEFT JOIN products Products ON Articles.product_id = Products.id WHERE Stocks.active =".$filter_active;
...
*/
        ...
        FROM `stocks` Stocks LEFT OUTER JOIN `details` Details on Details.`stock_id` = Stocks.id 
        LEFT OUTER JOIN `articles` Articles ON Articles.id = Stocks.article_id
        LEFT JOIN products Products ON Articles.product_id = Products.id WHERE Stocks.active = ".$filter_active;

 ...
 */
	...
	public function index($filter = null, $pagina = null, $filterid = null) {
		...
		if  ($user['catalogue_id'] != 0 ) {
			...
			//$this->loadModel('Servicesunit');
		

			//if ($user['show_muntador']) {
			...
			foreach ($cashOrderDetails as $num => $detail) {
				$art = $this->CashOrderDetail->Article->findById($detail['article_id']);
					...
					$per_box = $this->CashOrderDetail->query("select per_box, boxes_per_floor from catalogue_articles where catalogue_id = ".$user['catalogue_id']." and article_id = ".$detail['article_id']);
				//if ($user['show_muntador']) {

					/*$sql = "SELECT packaging.code FROM packaging, catalogue_articles ";
					$sql .= "where catalogue_articles.package_id = packaging.id and ";
					$sql .= "catalogue_articles.catalogue_id =".$user['catalogue_id']." and catalogue_articles.article_id = ".$detail['article_id'];

					$embalaje = $this->CashOrderDetail->query($sql);*/
					//print_r($embalaje);
				//}
					//$embalaje = $this->CashOrderDetail->query("select packaging.code from packaging where id = ".$detail['package_id']);
				//}

					$us_id = $detail['services_unit_id'];
					$us_name = $this->CashOrderDetail->query("select sigla from services_units where id = ".$us_id);


					//if ($user['show_muntador']) {	 
						$sql = "select cubicles.warehouse, (articles.height + packaging.mounted + 5) altura, ";
						$sql .= "round((CASE WHEN packaging.floor = 1 then SQRT(54.5 * 128 / cubicles.unities) ";
						$sql .= "	ELSE packaging.width END),2) longitud, ";
						$sql .= "round((CASE WHEN packaging.floor = 1 then SQRT(54.5 * 128 / cubicles.unities) ";
						$sql .= "	ELSE packaging.depth END),2) profundidad, ";
						$sql .= "packaging.code, packaging.floor, packaging.granel ";
						$sql .= "from catalogue_articles, articles, cubicles, packaging ";
						$sql .= "where catalogue_articles.catalogue_id = ".$user['catalogue_id']." and ";
						$sql .= "catalogue_articles.article_id = articles.id and ";
						$sql .= "catalogue_articles.package_id = packaging.id and ";
						$sql .= "cubicles.article_id = articles.id AND ";
						$sql .= "cubicles.package_id = packaging.id AND ";
						$sql .= "cubicles.is_active = 1 and ";
						$sql .= "articles.id = ".$detail['article_id'];

						$datos = $this->CashOrderDetail->query($sql);
						$datos = $datos[0];
						$altura = $datos[0]['altura'];
						$longitud = $datos[0]['longitud'];
						$profundidad = $datos[0]['profundidad'];
						$embalaje = $datos['packaging']['code'];
						$almacen = $datos['cubicles']['warehouse'];
						$es_piso = $datos['packaging']['floor'];
						$granel = $datos['packaging']['granel'];
					...
					//$us = $this->Servicesunit->findById($detail['services_unit_id']);
					//print_r($us).die;
//				}
				...
				$cashOrderDetails[$num]['ArticleCode'] = $art['Article']['code'];
				$cashOrderDetails[$num]['unidad_Servicio'] = $us_name[0]['services_units']['sigla'];
				$cashOrderDetails[$num]['unidades_caja'] = $per_box[0]['catalogue_articles']['per_box'];
				$cashOrderDetails[$num]['cajas_piso'] = $per_box[0]['catalogue_articles']['boxes_per_floor'];
				$cashOrderDetails[$num]['embalaje'] = $embalaje;
				$cashOrderDetails[$num]['almacen'] = $almacen;
				$cashOrderDetails[$num]['altura'] = $altura;
				$cashOrderDetails[$num]['longitud'] = $longitud;
				$cashOrderDetails[$num]['profundidad'] = $profundidad;
				$cashOrderDetails[$num]['es_piso'] = $es_piso;
				$cashOrderDetails[$num]['granel'] = $granel;
			...
			if (!empty($arrayCatalogues)) {
				$arrayIdioms = array(2 => 'cat',3 => 'fra',4=>'ing',5=>'ale');
			...
			switch ($filter) {
				...
				case 'search':
					...
					break;
				case 'distribution':
					$titleName = __('Distribución carris');
					break;
				default:

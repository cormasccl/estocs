...
<nav class="navbar navbar-default">
    <div class="container-fluid">
        <?php
        echo $this->Form->create('Articles', array('type'=>'get','class'=>'navbar-form navbar-left'));
        ?>

        
       
            <div class="form-group">
                <label for="ArticlesFilterName"></label>
                <input name="data[Articles][filter_name]" placeholder="<?=__('Buscar');?>..." class="form-control" type="text" id="ArticlesFilterName" value="<?=$filter_name;?>">
            
                <label for="ArticlesFilterStock"></label>
                <select name="data[Articles][filter_stock]" class="form-control" id="ArticlesFilterStock">
                    <?php
...
<!--<<div id="container">-->
    ...
    <table  id="container" class='table table-striped table-bordered table-articles'>
        <thead>
        <?php 
    ...
    { 
       ...
       if ($i == 0) {
        ...
        echo "<tr class='post ".$class."' id='item-".$row['Articles']['article_id']."'>";
        echo "<td>";
        echo "<a href='".SERVER."ImagesQualities/show/".$row['Articles']["article_id"]."'>";
        echo "<p>".$row['Articles']["article_name"]."</p><p class='nom_botanic'>".$row['Articles']['product_description']."</p></a>";
        echo "</td>";
        
        $str_price = str_replace(".", ",", $row['Articles']['article_price'])." €";
        echo "<td class='dreta td_unitats'>".$str_price."</td>";

        if ($row['Articles']["unities_stock"] == null ) {
            echo "<td class='dreta td_unitats'>0</td>";
        } else {
            echo "<td class='dreta td_unitats'>".number_format($row['Articles']["unities_stock"], 0, ',', '.')."</td>";
        }
...
<?php
    ...
    if (isset($next)) {
        ...
        $url = SERVER."Articles/selection/".$next;
        //if (isset($partner_id)) {
        //    $url .= "/".$partner_id;
        //}
